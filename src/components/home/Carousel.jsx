...
const Carousel = () => {
    ...
    return (
      <div className="symptom-wrapper padding-50 pos-top bg-dark-blue-2">
            ...
            >
              ...
              >
                {items.map((item, index) => (
                  <div
                    key={index}
                    className="symptom-content has-animation w-full flex-shrink-0 px-4 bg-blue-500 rounded-lg bg-[#fff]"
                    style={{ width: '303.333px', marginRight: '25px' }}
                    data-delay={index * 50}
                  >
                    <div className={`icon-div ${index + 1}-no`} ><i className={item.icon}></i></div>
                    <h4 className="mb-3 font-poppins  text-[15px] text-[#515184]  pt-[0.5em] ml-[3em] line-clamp-6">{item.title}</h4>
                    <p className='font-poppins-sans-serif  text-[15px] text-[#515184]  pt-[0.5em] ml-[3em] line-clamp-6'>{item.description}</p>
                  </div>
                ))}
              </div>
            </div>
